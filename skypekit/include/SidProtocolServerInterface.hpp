...
namespace Sid {
  ...
  namespace Protocol {
    ...
    struct ServerInterface {
      ...
      Status rd_parms_lst(CommandInitiator* thread, uint descriptor_begin, ...)
	  {
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = rd_parms_vlst(thread, descriptor_begin, parms);
			va_end(parms);
			return result;
	  }
      ...
      Status wr_response_lst(CommandInitiator* thread, const uint& rid, uint descriptor_begin, ...)
	  {
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = wr_response_vlst(thread, rid, descriptor_begin, parms);
			va_end(parms);
			return result;
	  }
...
#   ifdef SKYPE_SID_WITH_NO_PREENCODE
      ...
      Status wr_event_lst(CommandInitiator* thread, uint modid, uint event, uint descriptor_begin, ...)
	  {
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = wr_event_vlst(thread, modid, event, descriptor_begin, parms);
			va_end(parms);
			return result;
	  }
...
#   endif // SKYPE_SID_WITH_NO_PREENCODE
      ...
      Status wr_event_lst(CommandInitiator* thread, const uint& size_header, const char* header, uint descriptor_begin, ...)
	  {
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = wr_event_vlst(thread, size_header, header, descriptor_begin, parms);
			va_end(parms);
			return result;
	  }

      virtual Status wr_event_vlst(CommandInitiator* thread, const uint& size_header, const char* header, uint descriptor_begin, va_list parms) = 0;
      ...
      virtual Status end_property_change_event  (CommandInitiator* thread, bool immediate) = 0;
