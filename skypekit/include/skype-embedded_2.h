 ...
 *  \brief
 *  \nosubgrouping
 *  Address book entry. Encapsulates methods like GetIdentity, GetAvatar, SendAuthRequest, OpenConversation etc. Single
 *  contact can have additional phone numbers attached to it (ASSIGNED_PHONE1 .. ASSIGNED_PHONE3). Note that in the
 *  context of a conversation, Contacts are represented by Participant objects. @brief Contact member functions all
 *  return a Boolean indicating the success (true) or failure (false) of processing the request itself (transport,
...
public:
        ...
        enum PROPERTY {
                ...
                P_ASSIGNED_PHONE1 = 184         /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
                P_ASSIGNED_PHONE1_LABEL         /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
                P_ASSIGNED_PHONE2               /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
                P_ASSIGNED_PHONE2_LABEL         /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
                P_ASSIGNED_PHONE3               /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
                P_ASSIGNED_PHONE3_LABEL         /*!< See Contact::SetPhoneNumber method for more information. @n , type: Sid::String*/,
 ...
 *  portion of their name, e-mail address, language preferences, etc. <b>NB!</b> There is currently a known problem with
 *  complex contact searches, with only the last of the added search term being actually searched for. While we are
 *  investigating ways to fix this, contacts can be searched for by only one property at a time. @brief @n  @n
 *  <b>NB!</b> There exists a known problem with ContactSearch causing a noticeable memory leak. Under normal
 *  circumstances, this should not be a problem but massive amounts of contact searches are not infinitely sustainable
 *  without restarting the client.  @n  @n Contact search is asynchronous. ContactSearch::Submit is a non-blocking
 *  function that initiates the search. Upon finding a matching contact, ContactSearch::OnNewResult event gets fired,
 *  that gives you the reference to the discovered contact. You can get up to 100 matching contacts per search. Note
 *  that you will need to keep a live reference of the ContactSearch object while the search is doing its work.  @n  @n
 *  So, to perform a contact search:  @li create a contact search object @li specify search terms and conditions @li
 *  submit search @li in ContactSearch::OnNewResult callback, update your UI @li in ContactSearch::OnChange, check for
 *  terminal values of P_CONTACT_SEARCH_STATUS and update the UI accordingly. @n  @n When the search has done its job,
 *  the ContactSearch::P_CONTACT_SEARCH_STATUS property will go to one of the terminal values. @n  @n The terminal
 *  values are: @li FINISHED - the search has stopped. Note that this does not mean any matches were actually found. @li
 *  FAILED - the search has failed. @li EXTENDABLE - this state should be considered the same as FINISHED. The feature
 *  of extending long search results is about to be deprecated. It is still possible for search objects to occasionally
 *  reach that state, so it should be handled in the UI (as FINISHED), but the extending feature itself should not be
 *  implemented in your UI. @n  @n When a ContactSearch object reacges a terminal state, ContactSearch::Release has to
 *  be called to dispose of the object. @n  @n There are three methods to create the ContactSearch objects. @n  @n A)
 *  Skype::CreateIdentitySearch @n  @n This method takes a string argument and looks for exact matches against
 *  Contact::P_SKYPENAME property. So for example, identity search for "echo" will return 0 results and search for
 *  "echo123" will return exactly one.  @n  @n Identity in this case means skypename - contact search does not work with
 *  PSTN type contacts. However, it does work for SKYPE type contacts that have supplied P_PHONE_HOME, P_PHONE_OFFICE or
 *  P_PHONE_MOBILE values in their account data. To search for those, you will need to use complex search (see below).
 *  @n  @n Note that you should always check for boolean return value of the CreateIdentitySearch method. If the user
 *  submits a string that is not a valid skypename, the method will return false and the ContactSearchRef argument will
 *  return as NULL. @n  @n B) Skype::CreateBasicContactSearch @n  @n This method takes a string argument and looks for
 *  non-exact matches against both P_SKYPENAME and P_FULLNAME properties of the contact. If you intend to implement a
 *  simple, one-input search feature - this is the best method for you. The non-exact matching operates similarly to the
 *  SQL LIKE condition. @n  @n C) Skype::CreateContactSearch @n  @n This method enables you to implement advanced
 *  contact search, matching against multiple seach criteria. It takes no input arguments and expects search criteria to
 *  be added to the already constructed search object. @n  @n Criteria can be added with ContactSearch::AddStrTerm and
 *  ContactSearch::AddIntTerm methods. @n  @n These methods take Contact class porperty ID, condition, and the match
 *  pattern as inputs. @n  @n Only the following Contact properties can be used for search: @li P_SKYPENAME  @li
 *  P_FULLNAME @li P_BIRTHDAY (uint) @li P_GENDER (uint: 1-male, 2-female) @li P_LANGUAGES @li P_COUNTRY @li P_PROVINCE
 *  @li P_CITY @li P_PHONE_HOME @li P_PHONE_OFFICE @li P_PHONE_MOBILE @li P_EMAILS @li P_HOMEPAGE @li P_ABOUT @n  @n
 *  String searches are case insensitive, i.e. search for echo123 also matches ECHO123 @n  @n When adding multiple
 *  criteria, default behaviour is that the criterions are additive. I.e. a term skypename == "joe" followed by term
 *  country == "us" will result in intersection between all joes and everybody in US. @n  @n You can explicitly add an
 *  "OR" instead of "AND" between conditions, using the AddOr method. @n  @n By default, AND criteria are grouped
 *  together, before OR's, so that: @n  @n AddTerm(condition1) @n AddTerm(condition2) @n AddOr() @n AddTerm(condition3)
 *  @n AddTerm(condition4) @n  @n will result in the following logical statement: @n (condition1 AND condition2) OR
 *  (condition3 AND condition4) @n  @n However, you can add "global" critera, by using the add_to_subs argument of the
 *  AddXX methods. @n  @n AddTerm(condition1) @n AddTerm(condition2) @n AddOr() @n AddTerm(condition3) @n
 *  AddTerm(condition4, add_to_subs=true) @n  @n which would result in: @n (condition1 AND condition2 AND condition4) OR
 *  (condition3 AND condition4) @n  @n  @n Every one of the contact properties can only be used once, per search. For
 *  example, you cannot create a search for two different P_FULLNAME patterns. The &valid argument will still return tue
 *  if you do this, but the last criteria for any given property will override all previous ones. So, a search like
 *  this: @n  @n cs->AddStrTerm(Contact::P_FULLNAME, ContactSearch::EQ, "John Smith", isValid); @n cs->AddOr(); @n
 *  cs->AddStrTerm(Contact::P_FULLNAME, ContactSearch::EQ, "Ivan Sidorov", isValid); @n  @n will only return matches for
 *  "Ivan Sidorov" and none for "John Smith". @n  @n Some of the contact properties are automatically combined for
 *  purposes of search. @n  @n A search for P_SKYPENAME also returns matches from the P_FULLNAME property and vice
 *  versa. @n  @n So that this: @n cs->AddStrTerm(Contact::P_SKYPENAME, ContactSearch::EQ, "john.smith", isValid); @n
 *  @n ..and this: @n cs->AddStrTerm(Contact::P_FULLNAME, ContactSearch::EQ, "John Smith", isValid); @n  @n ..and this:
 *  @n cs->AddStrTerm(Contact::P_SKYPENAME, ContactSearch::EQ, "john.smith", isValid); @n cs->AddOr(); @n
 *  cs->AddStrTerm(Contact::P_FULLNAME, ContactSearch::EQ, "John Smith", isValid); @n  @n ..all search from both the
 *  P_FULLNAME and P_SKYPENAME fields. @n  @n  @n Before using ContactGroup::Submit to start the search, you should
 *  always check whether the search criteria ended up being valid. This you can do with ContactSearch::IsValid method.
 *  @n  @n As you probably noticed, each of the AddXX methods also return a validity check boolean. However, it is a
 *  better practice to do the overall check as well, even if all the individual search criteria ended up looking Ok. @n
 *  @n For example, lets take a search for contact's e-mail. This can be done with two different methods. Firstly we can
 *  use the ContactSearch::AddEmailTerm method. This method will actually validate whether the input is a valid e-mail
 *  address: @n  @n cs->AddEmailTerm ("test@test@test", isValid);  @n will return the isValid argument as false. @n  @n
 *  However, you can also add the e-mail search criterion as a simple string, like this: @n  @n
 *  cs->AddStrTerm(Contact::P_EMAILS, ContactSearch::EQ, "test@test@test@", isValid); @n in which case the isValid will
 *  return true. @n  @n However, if you then check entire search object with:  @n  @n cs->IsValid(isValid); @n  @n the
 *  isValid will correctly return false. @n 
 */
...
public:
        ...
        /** Releases and disposes of the ContactSearch object. This needs to be called when the ContactSearch object reaches a terminal state (FINISHED, FAILED, EXTENDABLE). @n  */
        bool Release(
        );
...
public:
        ...
        bool Ring(
                ...
                const Sid::String nonseWord = "" /*!< Nonse word used to provide end-point verification (ignore if you don't know what it means)*/
        );
        ...
        /** This method is now deprecated. It has been replaced by Participant::GetLiveSessionVideos, Participant::CreateLocalVideo and Participant::CreatePreviewVideo methods. @n  */
        bool GetVideo(
...
public:
        ...
        enum PROPERTY {
                ...
                P_GIVEN_DISPLAYNAME = 925       /*!< local name assigned via Rename, type: Sid::String*/,
                ...
                P_META_NAME = 910               /*!< deprecated, not used, type: Sid::String*/,
        ...
        /** Setupkey SETUPKEY_CHATDB_LIMIT_KB type:int default value:"0" @n Local chat database size limit in KB. @n This setup key is machine-specific and affects all local accounts. @n  */
        ...
        bool TransferLiveSession(
                ...
                const Sid::Binary context = Sid::Binary() /*!< Leave empty if you don't know what to use it for*/
        );
        ...
        /** Leaves the conference. Not applicable to dialogs. Note that this method is not intended for dropping out of live sessions. It removes the participant from the conversation entirely. For dropping out of live session (hang up), use Conversation::LeaveLiveSession or or Participant::Hangup instead. @n  */
        bool RetireFrom(
        );
...
public:
        ...
        enum PROPERTY {
                ...
                P_LEAVEREASON = 126             /*!< Leave reason for message of the RETIRED type, and STARTED/ENDED_LIVESESSION. @n                   Use for STARTED/ENDED_LIVESESSION is to provide simpler, enum based @n                   handling and deprecates the reason property (only set for dialogs), type: LEAVE_REASON*/,
        ...
        bool Edit(
                ...
                const Sid::String legacyPrefix = "" /*!< If the edit is received by a client not supporting displaying edits, @n                       it will be shown as a new message, prefixed by this parameter. @n                       Default value is a prefix in English.*/
        );
 ...
 *  this: @n  @n Let there be 3-way live conversation C and participants P1, P2 and P3. P1 is the local user. Remote
 *  participants P2 is capable of sending video. Remote user P3 is not capable of sending video. You would then end up
 *  with 3 video objects: V1, V2 and V0. @n  @li C->P1->V1-> outgoing video stream @li C->P2->V2-> incoming video stream
 *  1 @li C->P3-> no video object as participant P4 does not advertise supporting video @li V0-> local webcam preview -
 *  this is not attached to any particular conversation, however the corresponding video object can be retrieved with
 *  Skype::CreatePreviewVideo method. @n  @n Note that using V0 in a live conversation is not recommended. The video
 *  object you get back from Skype::CreatePreviewVideo is intended for webcam test and configuration part of your GUI.
 *  This video object is independant of any conversation. @n  @n As getting from a live conversation to running video
 *  streams involves three classes, it can be somewhat less than obvious. The basic state transition logic goes like
 *  this: @n  @n You start out with a Conversation, that suddenly becomes live @n  @n CONVERSATION::LOCAL_LIVESTATUS =
 *  IM_LIVE @n At this point you have access to participant list of that conversation. The next step will be to catch
 *  Partcipant property changes for PARTICIPANT::VIDEO_STATUS to see if any of the people in conversation have Video
 *  available. Note that you should not make assumptions on when this availability happens. Remote users may switch
 *  their video on-off as they please. @n  @n PARTICIPANT::VIDEO_STATUS = VIDEO_AVAILABLE @n If you get to
 *  VIDEO_AVAILABLE (not necessarily for all Participants), you can retrieve Video object for that Participant. Note
 *  that doing this is different, depending on whether the Participant is local user (outgoing video) or remote user
 *  (incoming video). @n  @n In case of local user: @li use Skype::CreateLocalVideo to retrieve the video object; @li
 *  use Conversation::AttachVideoToLiveSession to associate the video object with your live Conversation. @n  @n In case
 *  of remote user: @li use Participant::GetLiveSessionVideos to retrieve a list of video objects. As SkypeKit only
 *  supports one video stream per Participant, this list will contain either 1 or 0 elements. The video object you are
 *  looking for, either is or isn't the only element in that list. @n  @n At this point, you should check whether the
 *  Video::STATUS property is already AVAILABLE. If it is, you can start the video (send or receive) immediately.
 *  However, if it's not available yet - it doesn't mean it will not become available later. @n  @n To check for that
 *  you will need to handle Video::STATUS property changes to check for Video::STATUS becoming AVAILABLE. Once that
 *  happens, you can call Video::Start method to start sending or receiving video.  @n  @n In case of successful video
 *  call, the sequence of Video::STATUS and Participant::VIDEO_STATUS changes for each Participant and Video object
 *  should look like this: @n  @li Video::STATUS = AVAILABLE @li Video::STATUS = STARTING @li Video::STATUS =
 ...
 *  the Video object. @n  @n Also note that the Video object does not deal with video rendering. The Video object is
 *  just a control interface. To display incoming (or local preview) video, there exists a separate (shared memory
 *  based) interface, through which the SkypeKit runtime sends decoded video frames to the client UI. The server side of
 *  this interface exists in all desktop SkypeKit runtimes. The client side of the interface is currently implemented in
 *  C++ embedded and .NET wrappers. There is no client side interface for video rendering in Python or Java wrappers. @n
 *  @n However, you can take either the C++ based qt_videocalls example or the .NET SktVideoRenderer class sources and
 *  try your hand and porting the interface to your favourite combination of language and GUI framework. @n 
 */
...
public:
        ...
        enum PROPERTY {
                ...
                P_CONVO_ID = 1104               /*!< conference id to be able to identify remote/local video in the same call, type: ConversationRef*/,
        ...
        /** Switches between local webcams. This only works for video objects representing the local user. It is possible to switch to a different webcam on the fly, during a live session, i.e. you don't need to call Video::Stop and Video::Start while doing it. The easiest way to retrieve the local user is to call current live session's GetParticipants with PARTICIPANTFILTER.MYSELF. To get values for the deviceName and devicePath arguments, use Skype::GetAvailableVideoDevices method. Note that the Skype::GetAvailableVideoDevices should not be used immedately after account login. It will take several seconds to populate the video device list. You should handle the Skype::OnSkypeAvailableVideoDeviceListChange event to detect when the video device list is ready. @n  @n Note that the device switching will only work with webcams that are not already in use by some other application. For example, if you have a Skype desktop client with video running in parallel, while testing, the desktop client will "claim" its default webcam. That webcam will still show up in the video device list in SkypeKit-based app, but switching to it will not work. @n  */
        bool SelectVideoSource(
                const Video::MEDIATYPE& mediaType, /*!< Screensharing or webcam. @n */
                const Sid::String webcamName = "", /*!< Displayname of the webcam. @n */
                const Sid::String devicePath = "", /*!< Device ID (from the 2nd list you get back from the Skype::GetAvailableVideoDevices method) @n */
                const bool updateSetup = false /*!< Set true if you want to change the default value. @n */
        );
...
public:
        ...
        enum PROPERTY {
                ...
                P_OUTGOING_REPLY_TYPE = 1211    /*!< type: OUTGOING_REPLY_TYPE*/,
                P_STATUS = 191                  /*!< type: STATUS*/,
                ...
                P_REPLY_TO_NUMBER = 199         /*!< TYPE is outgoing: number that should receive the replies (DEPRECATED). TYPE is incoming: original source mobile number, type: Sid::String*/,
        ...
        enum REPLY_CAPABILITY {
                NOT_CAPABLE = 0               /*!< not 2-way capable*/,
        ...
        /** Call this when target has been delivered, to see what is the assigned number.  @n                     Calling before target has been delivered shows preliminary assigned number, which may not be allocated (ie when delivery fails) */
        bool GetTargetReplyNumber(
                const Sid::String& target,
                Sid::String& number
        );
        
        /** Fetch target reply type if toplevel Sms type is REPLY_TYPE_MIXED */
        bool GetTargetReplyType(
                const Sid::String& target,
                OUTGOING_REPLY_TYPE& type
        );
        ...
        /** Sms::P_OUTGOING_REPLY_TYPE property get accessor */
        bool GetPropOutgoingReplyType(OUTGOING_REPLY_TYPE& outgoing_reply_type);
 ...
 *  profile properties (local profile) is as follows: @n  @li P_FULLNAME, @li P_BIRTHDAY, @li P_GENDER, @li P_LANGUAGES,
 *  @li P_COUNTRY, @li P_PROVINCE, @li P_CITY, @li P_PHONE_HOME, @li P_PHONE_OFFICE, @li P_PHONE_MOBILE, @li P_EMAILS,
 *  @li P_HOMEPAGE, @li P_ABOUT, @li P_MOOD_TEXT, @li P_TIMEZONE, @li P_AVATAR_IMAGE, @li P_RICH_MOOD_TEXT @n  @n These
 *  can be set with SetIntProperty, SetStrProperty and SetBinProperty setter methods. Note that there are special
 *  methods for changing account online status (availability) and enabling/disabling auto-login - SetAvailability and
 ...
 *  login, with exception of creating a new account. @n  @n When creating a new account, setting the Account properties
 *  (the list above) needs to be done after the Account::Register has successfully finished. Attempts to set the
 *  properties before that will cause the setter methods to fail. There is one exception: The P_FULL_NAME property. That
 *  one needs to be set before calling the Register method. @n  @n The Account class has no default constructor and
 *  creating an Account instance is not the same as creating a Skype account. To create a Skype account: @n  @li obtain
 *  an Account instance by invoking Skype::GetAccount. This automatically sets P_SKYPENAME. @li set P_FULLNAME property
 *  @li invoke Account::Register to actually create the account @li set any additional profile-related properties. Skype
 *  recommends that you minimally set the following: @li their email address so that they can retrieve a lost password
 *  (P_EMAILS) @li at least one phone number (P_PHONE_HOME, P_PHONE_HOME, P_PHONE_OFFICE) @n 
 */
...
public:
        ...
        enum CAPABILITYSTATUS {
                NO_CAPABILITY = 0             /*!< Capability is not supported by the currently logged in SkypeKit client. @n */,
...
public:
        ...
        #define SETUPKEY_DB_PAGE_SIZE "*Lib/DbManager/PageSize"
        ...
        /** Returns the time as used in SkypeKit, in the form of a Unix timestamp (number of seconds since 1.1.1970). @n                   If the local system time is incorrect my more than one year, the time provided @n                   by the Skype network will be provided, which is correct. Therefore this function @n                   can be used to adjust the system time if set incorrectly (e.g. if set to 1.1.1970). */
        ...
        */
        ...
        bool GetConversationByIdentity(
                ...
                const bool matchPSTN = true /*!< The matchPSTN parameter changes the behaviour of the method for PSTN numbers. @n                     When matchPSTN is true, the method will look if there is a skypename that has this PSTN number associated @n                     (in his profile or as user-assigned phone number), and if there is, the method returns the conversation with that skypename. @n                     When matchPSTN is false, the method will simply return the conversation whose identity is that PSTN number.*/
        );
        ...
        */
        ...
        bool GetMessageListByType(
                const Message::TYPE& type, /*!< Type of messages requested. For POSTED_TEXT or POSTED_EMOTE, returns a list with both types*/
        ...
        * @param message link to the new Message object @n 
        ...
        * @param conversation link to the conversation object in which the new Message was created @n 
        */
        virtual void OnMessage(
                const MessageRef& message,
        ...
        /** This is now marked as deprecated and replaced by Skype::CreatePreviewVideo method. @n  */
        ...
        /** Retrurns video object for the local webcam specified in deviceName and devicePath arguments. The intended use of this method is in live conversations, for retrieving local user's video object, after it is determined that video object for the participant object for local user has video available. The video object can then be used for dual purpose, for local video rendering and sending the video stream out to remote parties. @n  @n Note that in order to actually send the video out to remote participants, you will need to associate the Video object with your live conversation, by calling the Conversation::AttachVideoToLiveSession method. Otherwise, the video will be able to render in your local GUI but remote participants will not be able to receive it. @n  @n For list of valid webcams (for deviceName and devicePath arguments), use Skype.GetAvailableVideoDevices method. Note that Skype.GetAvailableVideoDevices should not be called immediately after successful login. It will take several seconds for the runtime to populate the webcam list and until that happens, the video device lists will return empty. Instead of retrieving the video device list at startup, you should always handle the Skype::OnAvailableVideoDeviceListChange event and use the GetAvailableVideoDevices method to check if the list has now non-zero video devices in it. You should only enable video features in your GUI once this has happened. @n  */
        bool CreateLocalVideo(
                const Video::MEDIATYPE& type, /*!< either screen sharing or video stream from webcam @n */
                VideoRef& video, /*!< returns the video object. @n */
                const Sid::String deviceName = "", /*!< human readable device name (see Skype::GetAvailableVideoDevices method). @n */
                const Sid::String devicePath = "" /*!< video device ID (path) (see Skype::GetAvailableVideoDevices method). @n */
        );
        
        /** Retrurns video object for the local webcam specified in deviceName and devicePath arguments. This method should not be used for video objects in live conversations. The intended purpose of it is to provide a video object in your webcam configuration and test GUI. The video object returned by CreatePreviewlVideo cannot be used for sending out video streams. In live conversations, use Skype::CreateLocalVideo method instead. @n  @n For list of valid webcams, use Skype::GetAvailableVideoDevices method. Note that Skype::GetAvailableVideoDevices should not be called immediately after successful login. It will take several seconds for the runtime to populate the webcam list and until that happens, the video device lists will return empty. Instead of retrieving the video device list at startup, you should always handle the Skype::OnAvailableVideoDeviceListChange event and use the GetAvailableVideoDevices method to check if the list has now non-zero video devices in it. You should only enable video features in your GUI once this has happened. @n  @n This replaces the now deprecated Skype::GetPreviewVideo method. @n  */
        bool CreatePreviewVideo(
                const Video::MEDIATYPE& type, /*!< either screen sharing or video stream from webcam @n */
                VideoRef& video, /*!< returns the video object. @n */
                const Sid::String deviceName = "", /*!< human readable device name (see Skype::GetAvailableVideoDevices method). @n */
                const Sid::String devicePath = "" /*!< video device ID (path) (see Skype::GetAvailableVideoDevices method). @n */
        ...
        bool StartMonitoringQuality(
                const Sid::String& withUser, /*!< if empty, network test results would reflect status of local node only*/
                const bool& excludeNetworkTest
        );
        
        bool StopMonitoringQuality(
                const Sid::String& withUser,
                QUALITYTESTRESULT& result,
                const bool justStop = false
        );
        
        virtual void OnQualityTestResult(
                const QUALITYTESTTYPE& testType,
        ...
        /** creates and sends a CONFIRMATION_CODE_REQUEST message @n                   this sends a confirmation code to the number provided */
        ...
        /** creates and sends a CONFIRMATION_CODE_SUBMIT message @n                   this authorizes the number with the server for the purpose given in RequestConfirmationCode */
        ...
        bool GetISOLanguageInfo(
                ...
                Sid::List_String& languageNameList /*!< assumes UI has set correct language (UIPROP_LANGUAGE)*/
        );
        ...
        bool GetISOCountryInfo(
                ...
                Sid::List_String& countryNameList, /*!< assumes UI has set correct language (UIPROP_LANGUAGE)*/
                Sid::List_uint& countryPrefixList,
                Sid::List_String& countryDialExampleList
        );
        
        /** Get list of supported UI languages */
        bool GetSupportedUILanguageList(
        ...
        */
        ...
        bool RequestSSOToken(
                uint& request_id /*!< Request ID to attach a response to the request (or 0 on error)*/
        );
        
        /** Request web session cookies */
        bool RequestWebSession(
                uint& request_id /*!< Request ID to attach a response to the request (or 0 on error)*/
        );
        
        /** Request a web session for a user which is not logged in (yet) */
        bool RequestWebSessionWithPassword(
                const Sid::String& skypename,
                const Sid::String& password,
                uint& request_id /*!< Request ID to attach a response to the request (or 0 on error)*/
        );
        ...
        virtual void OnAuthTokenResult(
                const bool& success,
                const uint& request_id,
                const Sid::String& token
        );
        
        /** Request an auth token (oAuth access token or a RPS token) from the UI (Windows 8 in paricular) */
        virtual void OnAuthTokenRequest(
                const Sid::uint64& partnerId,
                const Sid::List_String& scopes
        );
        ...
        bool PutAuthTokens(
                const Sid::uint64& partnerId, /*!< Partner ID*/
                const Sid::List_String& scopes, /*!< List of oAuth scopes for the access token*/
                const Sid::String& accessToken, /*!< The oAuth access token*/
                bool& success,
                const uint expirationTime = 0, /*!< Expiration time for accessToken (optional)*/
                const Sid::String refreshToken = "" /*!< The oAuth refresh token (optional)*/
        );
        
        /** Link the Skype account (identified by skypename + password) with a partner account using oAuth accessToken */
        bool LinkAccountWithPartner(
                const Sid::uint64& partnerId, /*!< Partner ID*/
                const Sid::String& accessToken, /*!< oAuth access token of the partner account*/
                const Sid::String& skypename, /*!< Skype username - can be empty, a technical user will be created in that case*/
                const Sid::String& password, /*!< Skype password*/
                const Sid::String reason = "", /*!< Optional reason code for the linking (an arbitrary string)*/
                const bool allowSpam = false, /*!< Allow sending promotions and other material*/
                const bool allowSms = false /*!< Allow sending SMSs*/
        );
        
        /** Passes the (asynchronous) result of LinkAccountWithPartner() back to the caller
        * @param code result code - see the AUTH_RESULT definition
        * @param body contains the techical user skypename if it was created - otherwise contains the HTTP result body (may indicate errors)
        */
        virtual void OnAccountPartnerLinkResult(
                const AUTH_RESULT& code,
